...
Read in the text file with the <b>scan</b> function.Scanning the data shows that there are 407 lines. The character function returns the character value, while the new line <b>\n</b> is used as a seperator. Store the in the variable,<i>lines</i>.
```{r}
lines<-scan('currentsalbum.txt',what=character(),sep='\n')
```
...
We are now ready to finalize our wordcloud. View pallete colors with  the <b>display.brewer.all()<b> function. We are going to select the word and count column in word_freq. Our color pallete today is called <b>Set2</b>. 
```{r}
